...
Refreshed by `fn_refresh_views(false)` every 5 minutes via pg_cron (daily, coach month, rep month, client adherence) and `fn_refresh_views(true)` nightly (all). The app never selects the views directly; it calls the scoped accessors `fn_dashboard_daily/coaches/reps/adherence/heatmap/liability/sources/retention` and `fn_today_live`, which apply the caller's role and branch.
