import type { CoachDay, DaySession, Outcome } from "./coach";
import type { SyncStorage } from "../platform";

/**
 * Attendance taps that could not reach the server (no signal in the gym) wait here and are replayed in order.
 * Kept in the platform storage (localStorage on the web, an AsyncStorage mirror on the phone) so a reload does not lose them. Pure functions; the hook wires them up.
 */
