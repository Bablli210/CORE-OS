...
### `notification_deliveries` (0012, M7)
One row per WhatsApp / email / push notification that the notify function has picked up; `notification_id` is the primary key, so a notification is delivered at most once.
- Columns: `state` (claimed → sent → delivered → read, or retry / failed), `attempts`, `lease_until` (the claim's lease), `next_attempt_at` (retry backoff), `provider`, `provider_message_id` (unique; the WhatsApp webhook finds the row by it), `to_address`, `last_error`, and the `sent_at` / `delivered_at` / `read_at` times.
