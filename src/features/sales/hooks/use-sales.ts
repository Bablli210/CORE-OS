...
import { fetchNumbers, fetchQueue, fetchTeam, fetchToday, salesKeys } from "../queries/sales";
...
export const useNumbers = (branchId: string, month: string, membershipId: string) =>
  useQuery({ queryKey: salesKeys.numbers(branchId, month), queryFn: () => fetchNumbers(branchId, month, membershipId) });
