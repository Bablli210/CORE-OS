...
export type AuditRow = { id: number | string; occurred_at: string; kind: string; action: string | null; row_id: string | null; branch_id: string | null; actor: string | null; actor_id: string | null; old_row: Record<string, unknown> | null; new_row: Record<string, unknown> | null };
export type Audit = { tables: string[]; rows: AuditRow[] };
export type AuditSource = "events" | "audit" | "deliveries";
...
export const fetchAudit = async (f: { source: AuditSource; table?: string; actor?: string; from?: string; to?: string; search?: string }): Promise<Audit> =>
